...
async def messages_upsert(request: Request):
    ...
    agentOrchestratorService = dependencies.agentOrchestratorService()
    whatsAppOrchestratorService = dependencies.whatsAppOrchestratorService()
    try:
        # 1. Envia para o agente processar o que será feito!
        response_package = await agentOrchestratorService.process_message(messageupsertEntity.sender_id,messageupsertEntity.text)
